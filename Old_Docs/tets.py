...
def get_github_link(message):
    ...
    if github_link.startswith("https://github.com/") or github_link.startswith("github.com/"):
        ...
        username = soup.find('span', class_='p-name vcard-fullname d-block overflow-hidden').text.strip()
        url = f"https://api.github.com/users/{username}"
        response = requests.get(url)
        if response.status_code != 200:
            bot.send_message(message.chat.id,
                             f"Failed to download profile image for {username}. Status code: {response.status_code}")
            return
        data = response.json()
        ...
        for paragraph in doc.paragraphs:
            ...
            paragraph.text = paragraph.text.replace('<repos_num>', soup.find('span', class_='Counter').text.strip())
            paragraph.text = paragraph.text.replace('<followers_num>',
                                                    soup.find('a', {'href': f'/{username}?tab=followers'}).find('span',
                                                                                                                {
        ...
        img = Image.open(f'Users_docs/profile_img_{message.chat.id}.png').convert('RGB')
        img.save(f'Users_docs/profile_img_{message.chat.id}.png', 'PNG')
        run.add_picture(f'Users_docs/profile_img_{message.chat.id}.png', width=Cm(4), height=Cm(4))
        run.alignment = 1
        last_paragraph = doc.paragraphs[-1]
        run = last_paragraph.add_run()
        run.add_picture('Im_Stat_MilitaryGuineaPig.png', width=Cm(12), height=Cm(6))
        run.add_break()
        run.alignment = 1

        # Save the document and send a message to the user
